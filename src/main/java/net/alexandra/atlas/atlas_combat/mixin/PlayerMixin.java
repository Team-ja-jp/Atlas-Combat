...
public abstract class PlayerMixin extends LivingEntity implements PlayerExtensions, LivingEntityExtensions {
	...
	private static AttributeSupplier.Builder createAttributes(AttributeSupplier.Builder original) {
		return original.add(NewAttributes.BLOCK_REACH, !AtlasConfig.bedrockBlockReach ? 0.0 : 2.0)
	...
	public void injectCrit(Entity target, CallbackInfo ci, @Local(ordinal = 0) LocalFloatRef attackDamage, @Local(ordinal = 1) final float attackDamageBonus, @Local(ordinal = 2) LocalBooleanRef bl3) {
		...
		if(!AtlasConfig.sprintCritsEnabled) {
			isCrit &= !isSprinting();
		}
	...
	public boolean attackSpeedsMaxed() {
		...
		return util.compare(7.5F, AtlasConfig.swordAttackSpeed, AtlasConfig.axeAttackSpeed, AtlasConfig.woodenHoeAttackSpeed, AtlasConfig.stoneHoeAttackSpeed, AtlasConfig.ironHoeAttackSpeed, AtlasConfig.goldDiaNethHoeAttackSpeed, AtlasConfig.defaultAttackSpeed, AtlasConfig.tridentAttackSpeed, AtlasConfig.fastToolAttackSpeed, AtlasConfig.fastestToolAttackSpeed, AtlasConfig.slowToolAttackSpeed, AtlasConfig.slowestToolAttackSpeed);
	}
}
